 ...
 * @version $Id$
 */
//它实现了2个接口，它既有创建证书的实现，又有证书验证的实现，它是客户端使用的入口
public class LicenseManager implements LicenseCreator, LicenseVerifier {
	...
	private void setLicenseParam0(LicenseParam param) {
		...
		if (null == param.getPreferences())
			throw new NullPointerException();
		//验证对证书内容加密的密码，长度必须是大于6的数字字母混合形式
	...
	protected synchronized LicenseContent install(
			...
			throws Exception {
		//将lic的byte[]形式转换成GenericCertificate对象
	 ...
	 */
	//证书文件的验证分2步。
	//第一步验证签名，以防止证书被篡改或拿到的不是证书的初始的证书
	//第二步验证证书内容，查看是否符合规范
